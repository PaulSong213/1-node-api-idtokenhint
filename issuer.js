...
issuanceConfig.registration.clientName = "E. Zarate Hospital Verifier";
...
mainApp.app.get(`/api/issuer/issuance-request`, async (req, res) => {
  ...
  if (issuanceConfig.pin) {
    ...
    // } else {
    await sendPinToSMS(claimMobileNumber, issuanceConfig.pin.value);
    console.log(`pin: ${issuanceConfig.pin.value}`);
    // }
  }
  if (issuanceConfig.claims) {
    if (issuanceConfig.claims.blockchain_address) {
      issuanceConfig.claims.blockchain_address = claimBlockchainAddress;
    }
    if (issuanceConfig.claims.document_hash) {
      issuanceConfig.claims.document_hash = claimDocumentHash;
    }
    if (issuanceConfig.claims.recipientName) {
      issuanceConfig.claims.recipientName = claimDisplayName;
    }
  }
...
async function sendPinToSMS(claimMobileNumber, pin) {
  ...
  try {
    const message = await client.messages.create({
      body: `Your E. Zarate Hospital Verifier PIN is ${pin}`,
      from: 'whatsapp:+14155238886',
      to: 'whatsapp:+63' + claimMobileNumber,
    });
